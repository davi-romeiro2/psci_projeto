...
function previewImage(event) {
    ...
    if (file) {
        ...
        reader.onload = function (e) {
            ...
            container.querySelector('.profile-pic-text').style.display = 'none';
        };
...
function updateAgeField() {
    ...
    if (selected.value === 'crianca' || selected.value === 'adolescente') {
        ...
        const max = selected.value === 'crianca' ? 10 : 17;
...
document.addEventListener("firebaseReady", () => {
    ...
    registerBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            const generoSelect = formToValidate.querySelector('select');
            const telefoneResponsavel = formToValidate.querySelector('#phone')?.value || '';
            const telefoneUsuario = formToValidate.querySelector('#phone2')?.value || '';
            const tipoUsuario = !formKidTeen.classList.contains("hidden") ? "kid/teen" : "adult";
